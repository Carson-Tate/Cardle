...
// Yields index combinations of size k from [0, n), each as a fresh array.
// Standard lexicographic combination algorithm.
...
function evForDiscard(originalHand, drawPile, discardIndices) {
  ...
  let sum = 0;
  let count = 0;
  for (const drawIndices of combinationIndices(drawPile.length, discardIndices.length)) {
    const drawn = drawIndices.map((i) => drawPile[i]);
    sum += evaluateHand([...kept, ...drawn]).score;
    count++;
  }
  return sum / count;
}
...
export function solveOptimalDiscard(originalHand, drawPile, options = {}) {
  ...
  for (let k = minDiscards; k <= maxDiscards; k++) {
    for (const discardIndices of combinationIndices(originalHand.length, k)) {
      ...
      evByDiscard.push({ indices: discardIndices, ev });
    }
  }
