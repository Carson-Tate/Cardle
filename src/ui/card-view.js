 ...
 */
export function createCardElement(card, { faceUp = true, selected = false, locked = false, onClick } = {}) {
  ...
  if (faceUp) {
    if (card.rarity === 'joker') {
      ...
      el.setAttribute('aria-label', `${rankLabel(card.rank)} of ${suitName(card.suit)}${rarityLabel}${stateLabel}`);
    }
  } else {
