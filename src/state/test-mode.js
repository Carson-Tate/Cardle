import { dailySeed, hashSeed } from '../core/deck.js';

/**
 * Resolves how the board should run based on the URL.
 *
 * Normal play: `/` — today's deterministic seed, persisted to localStorage,
 * exactly one play per day (DESIGN.md §9.2).
 *
 ...
 */
export function resolveRunConfig(locationSearch = window.location.search, date = new Date()) {
  const params = new URLSearchParams(locationSearch);

  if (!params.has('test')) {
    return { isTestMode: false, seed: dailySeed(date), persist: true };
  }
